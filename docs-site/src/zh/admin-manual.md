...
访问 `http://<服务器>:1816`，使用操作员账号登录。初始管理员为 `admin`，口令来自
`{workdir}/private/admin-bootstrap-password` 中的一次性随机值（或
`TOUGHRADIUS_ADMIN_PASSWORD`）—— 请立即在 **账户设置**（右上角头像）中修改，该页
同时可编辑个人资料。密码至少 6 位，且不得使用历史默认口令。
