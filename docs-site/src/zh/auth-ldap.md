...
> **注意。** `radus_accept` 在每次认证成功时自增一次——裸 PAP/CHAP 流程与 EAP
> 流程都汇聚到同一个 Access-Accept 出口——因此可与上述拒绝计数器配对，用于成功率
> 或 SLO 观测。目录故障只会体现为 `radus_reject_ldap_error`，不会产生虚假的
> `radus_accept`，从而让成功与失败信号保持清晰分离。
