...
- 将 `server.pem` 与匹配的 `server.key` 作为服务器证书导入，例如本地名称
  `eap-server`。
- 将 `ca.pem` 作为客户端 CA 证书导入，例如本地名称 `eap-client-ca`。

然后打开 **系统配置 → RADIUS**，在 EAP 设置中选择这些名称（锚定
...
| **EAP-TLS 服务器证书**（`EapTlsServerCert`） | `eap-server` | 同上 | 三种隧道方式**都**出示它；该托管证书必须包含匹配私钥。 |
| **EAP-TLS 客户端 CA 证书**（`EapTlsClientCa`） | `eap-client-ca` | *(不使用)* | **EAP-TLS 必需**（校验客户端证书）；PEAP/TTLS 仅服务器认证，忽略它。 |
| **EAP-TLS 最低 TLS 版本**（`EapTlsMinVersion`） | `1.2` | `1.2` | PEAP/TTLS 无论如何都钉死在 **TLS 1.2**。 |

> 托管证书选择会在**两次握手之间**重新解析，因此可不重启 RADIUS 服务直接轮换导入的
> 证书。在所需托管证书未选择前，EAP 会**安全拒绝**（`ErrTLSNotConfigured`），而不会在
> 缺少信任锚的情况下放行 —— 半配置状态绝不会放任何人进来。
...
- **EAP-TLS 被拒，回包 `… handshake failed`** → 客户端证书未链到托管
  **`EapTlsClientCa`** CA（或选错了 CA）。用同一张 `ca.pem` 重签客户端证书，或选择正确
  导入的客户端 CA。
...
- **完全没有 EAP 挑战 / 立即拒绝** → 未选择 `EapTlsServerCert`（EAP-TLS 还需
  `EapTlsClientCa`），或所选服务器证书没有私钥，于是 EAP 安全拒绝。导入证书材料并选择
  托管名称即可，无需重启。
