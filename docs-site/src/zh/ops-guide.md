...
| **EAP（TLS/PEAP/TTLS）** | 系统配置 → `EapTlsServerCert`、`EapTlsClientCa`、`EapTlsMinVersion`、`EapTlsCipherProfile`（证书在「证书管理」中导入并按名称选择） | 未选择服务器证书即禁用基于证书的 EAP 方法；默认 `modern` 套件，CBC-only 客户端需 opt-in `legacy-rsa-cbc` + RSA 证书 |
...
设备、计费策略、用户、系统配置、操作员，以及托管证书（含私钥，确保恢复后
证书型 EAP 可继续工作）。服务器侧同时在 `{workdir}/backup/`
留存一份。**恢复** 可回导该文件。

> 快照**不包含**计费历史与在线会话。完整的灾备方案还需备份数据库本身
> （拷贝 SQLite 文件或使用 `pg_dump`）。

> **安全提示**：快照包含敏感凭据 —— RADIUS 用户密码、操作员密码哈希，
> 以及托管 EAP 证书的 PEM 私钥。备份文件应按机密数据妥善存储与传输。
