...
- [`.agents/skills/`](.agents/skills/) — reusable skill SOPs, one folder per skill (`.agents/skills/<name>/SKILL.md`). A **coordinator layer** drives the loop: `orchestrate-roadmap` (entry role — on an "auto-delegate development" command it selects the next subtask, dispatches the matching skill, enforces gates, opens a PR), `review-pr` (independent, CI-anchored review gate that requests changes via labels/comments and auto-merges only when approved + green), and `groom-roadmap` (self-iterates the roadmap/plan after each merge). The execution SOPs it orchestrates: add vendor VSA, add EAP method, add admin API, add React Admin resource, add config schema, add acceptance test, sync upstream radius, reference RFC, align checklist, write Go tests. Pick the matching skill before starting a task type.
