...
def parse_auth_packet(datagram,(host,port),dictionary=None,client_config):
...
def parse_acct_packet(datagram,(host,port),dictionary=None,client_config):
    if host in client_config.defaults:
        client = client_config.defaults[host]
...
def process_acct_reply(req, prereply):
    ...
    try:
        ...
        for module in (response_logger,)
            ...
            if not req.VerifyReply(reply):
                errstr = u'[User:%s] The accounting message failed to check. \
                Check that the shared key is consistent'% username
                raise packet.PacketError(errstr)
    except:
