...
func (s *RadsecPacketServer) Serve(conn net.Conn) error {
	...
	if err != nil {
		log.Errorf("radius: error fetching from secret source: %v", err)
		return err
	}

	if len(secret) == 0 {
		log.Errorf("radius: empty secret returned from secret source")
		return err
	}
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				log.Infof("radius: connection closed by client %s", conn.RemoteAddr())
				return err
			}
			if _, ok := err.(net.Error); ok {
				log.Infof("radius: connection error %s: %v", conn.RemoteAddr(), err)
				return err
			}
			log.Errorf("radius: unable to parse packet: %v", err)
			continue
		}
