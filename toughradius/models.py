...
class TrBas(DeclarativeBase):
    ...
    nas_id = Column(u'nas_id', Unicode(length=128), nullable=True, doc=u"nas id")
...
class TrAcl(DeclarativeBase):
    """黑白名单 0 白名单 1 黑名单"""
    __tablename__ = 'tr_acl'
    ...
    action = Column('action', Unicode(length=19),  nullable=False,doc=u"黑白名单类型")

class TrAccountOrder(DeclarativeBase):
    """
    订购信息表(交易记录)
...
class TrAccount(DeclarativeBase):
    ...
    address = Column('address', Unicode(length=128), nullable=False,doc=u"装机地址")
    ...
    ip_pool = Column('ip_pool', Unicode(length=32),doc=u"ip pool")
...
class TrProduct(DeclarativeBase):
    ...
    policy = Column('policy', INTEGER(), nullable=False,doc=u"资费策略")
    status = Column('status', SMALLINT(), nullable=False,doc=u"资费状态")
    fee_days = Column('fee_days', INTEGER(),doc=u"买断授权月数")
