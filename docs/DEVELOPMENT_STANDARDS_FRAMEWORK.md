...
- `semantic_search`：语义检索概念、函数、架构描述
- `grep_search`：通过精确字符串/正则定位实现细节
- `file_search`：按目录模式快速定位相关文件
- `list_code_usages`：查找函数、类型、接口的引用位置

**搜索模式示例:**

```text
# 精准+上下文
semantic_search "Huawei vendor attribute parsing" in internal/radiusd/vendors
