...
- [x] **FIX-002｜越权：写操作与 backup/restore 增加角色鉴权**
...
- [x] **FIX-005｜`UpdateBind` VLAN 写错字段 Bug**
...
- [x] **FIX-007｜EAP 状态加 TTL 清理 + 移除死代码 `EapStateCache`**
...
- [x] **FIX-014｜抽取分页/排序/过滤公共 helper**（根治 SQLi 类不一致）
  - 位置：`users.go`/`nas.go`/`profiles.go`/`operators.go`/`settings.go`
- [x] **FIX-015｜完成仓储层去 `Deprecated`**，使非 Deprecated 路径成为主路径
  - 位置：`internal/radiusd/radius.go`
- [x] **FIX-016｜合并/厘清 `registry/` 与 `vendors/` 两套注册表职责**
- [x] **FIX-017｜补全厂商解析**：Huawei VLAN 解析去 stub；明确「字典支持 ≠ 解析支持」
  - 位置：`internal/radiusd/plugins/vendorparsers/parsers/`
- [x] **FIX-018｜移除重复 DevTools well-known 路由注册**
  - 位置：`internal/webserver/server.go:107,110`
- [x] **FIX-019｜统一 JWT 库版本**（v4 / v5 二选一）
  - 位置：`internal/webserver/server.go`、`internal/adminapi/auth.go`
- [x] **FIX-020｜修正拼写**：`EapMethad`、`Sendresponse` 等
- [x] **FIX-021｜实现 `SaveSettings`（移除空 placeholder）**
  - 位置：`internal/app/app.go:211`
- [x] **FIX-022｜移除生产代码中的测试后门分支**
  - 位置：`internal/adminapi/auth.go:105`（`current_operator` 上下文读取）
- [x] **FIX-023｜补测试**：`settings.go` CRUD、授权负向用例、`UpdateBind`、记账错误分支、缓存淘汰路径
...
> 状态：**全部 23 项已完成并合并到 `main`**（2026-06）。每项修复均独立成 PR、经 CI 校验后通过合并队列（squash）合并。
