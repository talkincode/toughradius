...
- [ ] M14.6 `test/integration/` LDAP 端到端验收用例（CI 自动执行）：以 OpenLDAP service 容器（仿照 integration job 的 postgres service）+ 种子 LDIF，让已启动的 RadiusService 经真实 bind 完成裸 PAP 与 `EAP-TTLS/PAP` 认证；覆盖 bind 成功放行、口令错误拒绝、目录不可达→`radus_reject_ldap_error`、非 PAP 族拒绝。**风险提示**：改动共享 CI integration job（误配会让 integration 红灯阻塞合并队列中所有 PR），需单独一轮、谨慎验证（`add-acceptance-test`）。
