...
英文默认版：[docs/roadmap.md](roadmap.md)

> 本路线图是 ToughRADIUS 的中文详版长期开发计划；英文默认版与 agent 任务入口见 [`docs/roadmap.md`](roadmap.md)。本文件与功能清单 [`docs/feature-checklist.md`](feature-checklist.md) 强绑定。
> 所有里程碑任务必须引用至少一个功能编号（如 `TR-F004`），不在清单中的方向先更新清单再排期。
> 本文件保留详细交付记录与中文说明；agent 默认从英文版 `docs/roadmap.md` 读取当前可执行任务。运行方式与护栏见 [`.agents/README.md`](../.agents/README.md)。
...
4. 严禁触碰非目标方向（见功能清单 `TR-N001`~`TR-N006`）：支付/订单、CRM/工单、通用监控平台、多租户 SaaS、重写协议栈或管理框架、托管式 Captive Portal 产品。
...
7. **交付记录密度**：已交付子任务只保留结果、证据、剩余风险与可追溯入口；长篇实现过程优先沉淀到 PR、提交记录或后续 changelog，避免路线图退化为执行日志。
8. **任务选择规则**：自动委托默认选择最高优先级里程碑中第一个未勾选且未标注 `阻塞` / `待证据` 的子任务；被阻塞任务保留原因，但不阻塞后续可执行项。
...
| M10 | EAP-TLS 1.3 / RFC 9190 升级 | TR-F004 | P2 | 进行中（M10.1 #562、M10.2 #564、M10.3 #607 已交付，下一个 M10.5 身份保护 / 匿名 NAI） |
...
- [x] M4.12 按模块增量补齐导出标识符 godoc 注释与包注释（顺序建议：`internal/adminapi` → `internal/radiusd` → `pkg`），并探索 lint 度量（`TR-F024`）<br/>进行中（增量交付）：批次 1 已补齐 `internal/adminapi` 框架层——新增包注释（`adminapi.go` 顶部，标准库风格，说明 /api/v1 管理 API 定位、handler 无状态从 echo context 取依赖、统一 `Response`/`ErrorResponse` 信封与 `RequireLevel` 鉴权模型）、`Init`/`GetAppContext`/`GetDB`/`GetConfig` 契约注释（读取来源、中间件契约、缺失即 panic 的程序性错误语义、按 key 回落），及 `Meta`/`Response`/`ErrorResponse` 信封类型契约；并以 `nas.go`（`ListNAS`/`GetNAS`/`CreateNAS`/`UpdateNAS`/`DeleteNAS`）为资源处理器范本：首段散文化契约（HTTP 方法/路径、鉴权级别、参数与默认值/钳制、错误码与响应体形状、SQL 注入护栏），保留既有 Swagger `@` 注解并以空注释行与散文分段（`go doc` 首段即干净摘要）。门禁：`gofmt`、`go build ./...`、`go vet`、`go test ./internal/adminapi/...`、golangci-lint v2.12.2 对该包 0 问题。余 `internal/adminapi` 其余资源处理器（users/profiles/sessions/accounting/dashboard/operators/nodes/settings/system_backup/system_logs/session_actions/auth/users_import）及 `internal/radiusd`、`pkg` 待后续批次续接；lint 度量（按包 ratchet 开启 ST1000/ST1020/ST1021 或 revive `exported`）待该包补齐后再探索。<br/>批次 2：已补齐 `internal/adminapi/profiles.go` 资源——为 5 个导出 handler（`ListProfiles`/`GetProfile`/`CreateProfile`/`UpdateProfile`/`DeleteProfile`）与 2 个导出请求 DTO（`ProfileRequest`/`ProfileUpdateRequest`）补齐标准库风格契约注释（HTTP 方法/路径、鉴权级别——读端开放/写端 `requireAdmin`、参数默认值与钳制、错误码与 `Response`/`Meta` 信封形状、`ORDER BY` 白名单防注入、缓存失效语义），保留 Swagger `@` 注解并以空注释行分段（`go doc` 首段即干净摘要）；顺带修正 `toRadiusProfile`/`registerProfileRoutes` 的破碎英文注释。门禁：`gofmt`、`go build`、`go vet`、`go test ./internal/adminapi/...`、golangci-lint v2.12.2 对该包 0 问题。余 `internal/adminapi` 其余资源（users/users_import/sessions/session_actions/accounting/dashboard/operators/nodes/settings/system_backup/system_logs）及 `internal/radiusd`、`pkg` 续接。<br/>批次 3：已补齐 `internal/adminapi/sessions.go` 在线会话资源——为 3 个导出 handler（`ListOnlineSessions`/`GetOnlineSession`/`DeleteOnlineSession`）与 `registerSessionRoutes` 补齐标准库风格契约注释：列表的分页/钳制、过滤字段（精确 vs 转义 LIKE 子串）、多格式时间窗、`ORDER BY` 白名单防注入与分页信封；`DeleteOnlineSession` 强制下线语义（删除在线记录 + 异步尽力而为发送 RFC 5176 Disconnect-Request、5s 超时、ACK/NAK 仅记日志不影响响应、NAS 缺失则仅删库告警）与错误码（`INVALID_ID`/`NOT_FOUND`/`DELETE_FAILED`）；并据实记录鉴权级别（读 + 强制下线对任意已认证操作员开放，而 `POST /sessions/:id/disconnect`、`/coa` 由 `requireAdmin` 守护）。门禁：`gofmt`、`go build`、`go vet`、`go test ./internal/adminapi/...`、golangci-lint v2.12.2 对该包 0 问题。<br/>批次 4：已补齐 `internal/adminapi/dashboard.go` ——为 `GetDashboardStats` 写明聚合契约（一次请求返回实时计数 + 今日量值 + 7 天认证趋势 / 24 小时流量 / 在线用户按 profile 分布 + IPv6 采用率三类时序；今日以本地零点起算、趋势/流量按桶零填充、流量按 1024³ 折算 GB、`TodayAuthCount` 为按当日新会话估算而非独立认证计数器、单段查询失败仅记日志回落零值、对任意已认证操作员开放），并将 `DashboardStats`/`DashboardAuthTrendPoint`/`DashboardTrafficPoint`/`DashboardProfileSlice` 类型头注释升级为标准库风格契约、补齐 `DashboardProfileSlice` 字段注释（`DashboardIPv6Stats` 此前已具完整 godoc，未改）。门禁：`gofmt`、`go build`、`go vet`、`go test ./internal/adminapi/...`、golangci-lint v2.12.2 对该包 0 问题。
  ...
  <br/>批次 18：完成 lint ratchet 收口——先以 `go run honnef.co/go/tools/cmd/staticcheck@latest -checks ST1000,ST1020,ST1021,ST1022 ./...` 验证全仓导出标识符与包注释风格 clean，再在 `.golangci.yml` 移除 `-ST1000` 豁免，把包级注释检查纳入常态门禁（与已启用的 ST1020/ST1021 一致）；全量门禁 `go build ./...`、`go test ./...`、`go run github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.2 run` 通过。至此 `M4.12` 的“增量补齐 + lint 度量探索”目标闭环完成。
...
- [x] M10.3 `close_notify`（TLS 1.3 语义差异）——已交付（#607）：成功点（`stateKeyPendingSuccess`）之后，真实请求方通常直接关闭其 TLS 栈并发出 `close_notify` 警告，而非按 RFC 9190 图 1 回送裸 ACK；旧实现把这类非 ACK 分片当作 `ErrTLSUnexpectedFragment` 拒绝，使一次本应成功的认证变成 Access-Reject。修复仅针对**纯 EAP-TLS**（以 `onApplicationData == nil` 唯一标识；PEAP/TTLS 保持严格处理不变）：非 ACK 分片经新增 `finishAfterPeerClosure` 用 `engine.ReadApplication` 判定——`io.EOF`（干净 `close_notify`）→ 放行并照常导出 MS-MPPE 密钥；非 EOF 读错误（TLS *error* 告警）→ `ErrTLSHandshakeFailed` 拒绝；读到多余应用数据 → `ErrTLSUnexpectedFragment` 拒绝（fail-closed）。RFC 锚点：RFC 9190 §2.1.4（`close_notify` 为 warning 级关闭、非错误）、§2.5（仅 TLS *error* 告警才是失败结果指示）、RFC 8446 §6.1。4 个单测（1.2 关闭→放行、1.3 先读 0x00 再关闭→放行、多余应用数据→拒绝、损坏记录→拒绝）+ `test/integration/` 子用例「tls 1.2 client close_notify still authenticates」（CI 背书）；`-race` 全绿。**身份保护（匿名 NAI）拆分至新增 M10.5**，因其需重排认证流水线（`stageLoadUser` 目前以外层 User-Name 先行解析账户）。
- [x] M10.4 单元测试 + `test/integration/` TLS 1.3 端到端验收用例（CI 自动执行）——已由 M10.1–M10.3 增量交付满足：`test/integration/eap_tls_test.go` 现含 6 个 CI 子用例，覆盖 TLS 1.3/1.2 握手与版本回退、MSK→MS-MPPE 密钥派生、`close_notify` 认证，均随各功能子任务落地并在 CI 自动执行；本条无独立剩余交付物，折叠收尾。
- [ ] M10.5 身份保护 / 匿名 NAI（RFC 9190 §2.1.7 / §2.1.8）：允许请求方在外层 EAP-Identity 使用匿名 NAI（如 `anonymous@realm`），服务器改由 TLS 隧道内的**客户端证书**解析真实账户身份，而非外层 User-Name。需重排认证流水线：当前 `stageLoadUser` 在 EAP 运行前即以外层 User-Name 解析账户，故须在 TLS 握手取得证书身份后再做账户绑定/加载。保持与非匿名客户端向后兼容；单测 + `test/integration/` 匿名 NAI 端到端用例（CI 背书）。
...
- [x] M13.3 收编功能清单 / 路线图 / RFC 索引为站点章节，建立中英文交叉链接<br/>批次 1 已收编 **RFC 索引** —— 新增双语章节 `docs-site/src/{en,zh}/rfc-index.md`（精选、面向实现的协议/RFC 索引，将 RADIUS 核心 / RADIUS-EAP 集成 / EAP 框架与方法 / 动态授权 / 安全传输 / 厂商属性 / IPv6 各 RFC 映射到代码与里程碑，并列出路线图标准 RFC 9190+9427/7170+9930/5931 对应 M10/M11/M12，互为中英文交叉链接），登记进 `SUMMARY.md`（置于 文档地图 之后），更新 `documentation-map` 的 RFC 索引条目指向手册章节（标注「权威/原始目录」）。引用经校正：EAP-MD5 归 RFC 3748 §5.4（非 RFC 3851），EAP-PWD 归 RFC 5931（非 RFC 7542=NAI）；并在 `docs/rfcs/README.md` 顶部加指向手册章节的指针。门禁：`mdbook build docs-site` 通过、lychee `--offline` 0 错误。余 功能清单、路线图 两份待后续批次。<br/>批次 2（收尾，**完成本子任务**）：经设计判断，**功能清单与路线图保持「链接而非迁入」**——二者是由专用技能持续改写的 *living docs*（`docs/feature-checklist.md`/`.en.md` 由 `align-feature-checklist` 维护、`docs/roadmap.md` 由 `groom-roadmap` 每轮改写，且 `AGENT.md` 护栏与工具链直接引用其路径）；若将其作为站点章节迁入，要么内容重复随时漂移，要么迫使上述技能改写目标到 `docs-site/`，二者皆无收益且增加耦合。改为：在双语 `documentation-map` 中提供中英文交叉链接（指向 `docs/feature-checklist.md`·`.en.md` 与 `docs/roadmap.md`）并显式标注其为 living docs（见 doc-map「迁移计划」段），等同此前 M13.2 对 `README` 的处置先例（保留为 GitHub 首页 + 链接，不指针化/不迁入）。至此 RFC 索引已迁入为真实双语章节、功能清单/路线图以交叉链接对外可达，M13.3 收尾，**M13 里程碑全部子任务交付**。门禁：本批次仅改 `docs/roadmap.md`（无 `docs-site/` 改动），`mdbook build docs-site` 与 lychee `--offline` 仍 0 错误。
...
- [x] M13.8 厂商场景实战手册（Scenario Cookbook，用户指令：「尽量提供靠近实际应用运维场景的范例指导」）—— 在 `vendor-guide`（属性参考卡）之上新增「场景实战」层：每个场景采用统一的「需求 → ToughRADIUS 侧配置 → 设备侧配置 → 验证 → 排障」五段式，ToughRADIUS 侧每条声明锚定真实代码（enhancer 实际下发的属性、checker 实际执行的拒绝逻辑），设备侧配置标注「示例，以实际固件为准」。分批交付：**批次 1 = MikroTik RouterOS 旗舰场景**（PPPoE 宽带 ISP 分级套餐 + 地址池 + 到期断网 + 并发限制；Hotspot + MAC 认证；CoA/强制下线与 FUP——按代码实际能力，CoA 仅 Session-Timeout/Filter-Id，变速走 Disconnect 重连）**[已交付]**；**批次 2 = 华为 BRAS / NetEngine**（PPPoE/IPoE 分级套餐 + 峰值速率 + AAA 域——限速四元组锚定 `huawei_enhancer.go`，bit/s = Kbps×1024、峰值=平均×4、Input=上行/Output=下行；线路防盗用 MAC+VLAN 绑定锚定 `mac_bind_checker`/`vlan_bind_checker` + 双栈静态 IPv6；CoA/强制下线与 FUP）**[已交付]**；**批次 3 = H3C / 中兴 / 爱快 / Cisco 差异速查**（合并为一章，避免重复旗舰剧本：逐厂商只列「下发的限速属性 + 单位倍率 + MAC/VLAN 解析 + 适用旗舰场景指针」——H3C 锚定 `h3c_enhancer.go`/`h3c_parser.go`，中兴 `zte_enhancer.go`/`zte_parser.go`，爱快 `ikuai_enhancer.go`+默认解析器=无 VLAN，Cisco 无 enhancer=仅标准属性；附厂商能力矩阵）**[已交付]**。门禁：`mdbook build` + lychee `--offline`（含 `--include-fragments`）0 错误，中英双语对称。
...
- [x] M14.2 接入认证流水线作为 PAP 族校验后端：裸 PAP、`EAP-TTLS/PAP` 复用同一入口；非 PAP 族（CHAP 系 / EAP-MD5 / PEAP-MSCHAPv2）明确拒绝并记录可诊断原因，不在协议入口分叉<br/>**已交付**：①新增 `internal/radiusd/ldap_backend.go` 适配器 `ldapCredentialBackend`，把 M14.1 的 `ldapauth.Verifier` 桥接进 RADIUS 认证流水线；同时实现 `eap.PasswordProvider` 与新增的 `eap.CredentialVerifier`（编译期 `var _` 断言）。后端每次尝试都读 `ldap.*` 配置（运行时改配置即时生效），经 `s.AppContext()`（结构化满足 `ldapauth.Settings`，无导入环）取值；`nil` settings 退化为「未启用」。②**集中式非 PAP 拒绝（不在协议入口分叉的关键设计）**：LDAP 启用时本地 `RadiusUser.Password` 往往为空/失效，任何挑战响应方法（CHAP/MS-CHAP/MS-CHAPv2/EAP-MD5/PEAP-MSCHAPv2）若仍以本地口令推算期望响应，会基于空密钥**误判通过任意用户**——因此让「取口令」边界 LDAP 感知：`GetPassword` 在「启用 + 非 MAC」时返回诊断性 `AuthError`（空口令），于是 4 个 EAP 挑战响应处理器（`mschapv2_handler`/`ttls_mschapv2`/`peap_inner`/`md5_handler`）**自动拒绝**（错误向上传播），仅 `ttls_inner.go` 内层 PAP 获得正向 bind 分支；裸路径在 `auth_plugin_runner.go::validateCredential` 分流（PAP→bind，非 PAP/空口令→诊断拒绝），故裸 CHAP/MSCHAP 校验器根本不会被触达。MAC 认证（`isMacAuth=true`）两路均绕过 LDAP。③**接线**：`eap/interfaces.go` 新增 `CredentialVerifier` 接口 + `EAPContext.Verifier` 字段；`eap/coordinator.go` 构建 `EAPContext` 时类型断言 `pwdProvider.(CredentialVerifier)` 写入 `Verifier`（无签名变更，既有 ~8 处 `NewCoordinator` 测试调用与 `mockPasswordProvider`/`DefaultPasswordProvider` 因不实现该接口→`Verifier=nil`→行为不变）；`eap_helper.go` 把 `NewDefaultPasswordProvider()` 换成 `newLDAPCredentialBackend(radiusService.AppContext())`。④**错误/指标映射**：`mapLDAPVerifyError` 纯函数——`ErrInvalidCredentials`/`ErrUserNotFound`/`ErrEmptyCredential`→`radus_reject_passwd_error`（口令拒绝）；`ErrUnavailable`/`ErrNotConfigured`/`ErrDisabled`/未知→`radus_reject_ldap_error`（后端故障，不把目录不可达伪报为「口令错误」）。⑤**测试（均经变异验证 = 删守卫则测试失败）**：`ldap_backend_test.go`（`Active` 三态、`GetPassword` 启用+非MAC→空+AuthError 安全守卫、`mapLDAPVerifyError` 全 sentinel、`verifyRequestPAP` 无 PAP/空用户→诊断拒绝 + PAP 路由进 bind、`VerifyCleartext` 未配置/空口令映射）；`ttls_inner_test.go` 扩展端到端 TLS 握手 harness 加 3 例（LDAP bind 成功→放行且仍导出 MS-MPPE 密钥、LDAP bind 拒绝须压过「本地口令恰好匹配」、未启用 verifier 回退本地比较且零调用）；并补 M14.1 评审遗留项（`ldapauth_test.go`：服务账号 bind 收到 code-49 须映射 `ErrUnavailable` 而非 `ErrInvalidCredentials`——通用错误用例无法捕获把 `mapBindError` 误用到服务 bind 的重构，唯 code-49 能区分两路，已变异验证）。**部署模型**：本地 `RadiusUser` 行仍必需（提供授权：套餐/限速/到期/并发/地址池，认证前由 `stageLoadUser` 加载），LDAP 仅替换口令校验，不自动开户；全局 `ldap.Enabled` 开关，无逐用户认证源字段（属未来增强）。门禁：`gofmt`、`CGO_ENABLED=0 go build ./...`、`CGO_ENABLED=0 go test ./...`、golangci-lint v2.12.2 全绿（未触前端，跳过 web build）。连接复用/池化与更丰富的可观测归 M14.3。
- [x] M14.3 可观测与拒绝语义：超时、bind 失败与目录不可达的拒绝语义与**按原因计数的指标**（复用 `AuthError` + metrics）<br/>**已交付**：①**修复一处既有可观测盲区**——全仓审计（`grep IncRadiusMetric`）发现 RADIUS 认证计数器此前仅在两处自增：`auth_stages.go::logEAPFailure`（EAP 拒绝）与 `radsec_server.go`（RadSec 饱和）；**裸 PAP/CHAP 拒绝路径从不自增任何拒绝计数器**——`radius_auth.go::logAndReject` 早已按错误算出 `metricsKey` 却只写日志、从不调用 `app.IncRadiusMetric`。后果：以 LDAP 为后端的部署（多为裸 PAP）即便目录宕机，`radus_reject_ldap_error` 也恒为 0，直接架空 M14 的「可观测」目标。修复=在 `logAndReject` 算出 `metricsKey` 后补一行 `app.IncRadiusMetric(metricsKey)`。②**不重复计数**（已读 `auth_stages.go` 核验）：EAP 拒绝在 `stageEAPDispatch` 内由 `logEAPFailure` 自增后 `return nil`、不再下沉到 `pipeline.Execute`→`logAndReject`，故无双计；裸路径经 `stagePluginAuth` 返回错误→`processAuthError`（只跑守卫、不计数）→`logAndReject`（唯一计数点）。③**分类守卫**：`*AuthError.MetricsKey()` 即其 `MetricsType`，故经 M14.2 的 `mapLDAPVerifyError` 后，目录不可达→`radus_reject_ldap_error`、口令错误→`radus_reject_passwd_error`，二者不混淆（测试明确断言 LDAP 故障不会自增口令错误计数器，避免污染告警面板）。④**测试（变异验证）**：`radius_auth_metrics_test.go`——`logAndReject` 对 LDAP-error / passwd-error / 无分类（回退 `radus_reject_auth_drop`）三类错误各自增对应计数器、并写出 Access-Reject；删除自增行→测试失败（已验证）。**超时**已在 M14.1 交付（`cfg.Timeout` 5s 兜底，约束拨号与每次操作）；**拒绝语义**已在 M14.2 交付（`mapLDAPVerifyError` 不把目录不可达伪报为口令错误）。门禁：`gofmt`、`CGO_ENABLED=0 go build ./...`、`CGO_ENABLED=0 go test ./...`、golangci-lint v2.12.2 全绿（未触前端）。**连接复用/池化、重连、请求上下文传播**经 dev-requirements-audit 判定证据不足、且 naive 池化不安全（bind 身份随每次认证变化，复用连接会以错误身份服务下一请求），拆出独立证据驱动子任务 **M14.5**。
- [x] M14.4 双语文档章节（用户手册 / 运维指南内新增「LDAP / AD 认证后端」章节，明示「仅 PAP 族」、bind 两模式、StartTLS/LDAPS、配置项逐项说明、本地 RadiusUser 行仍必需=仅替换口令校验、安全注意事项）——**注**：单元测试覆盖（mock / fake-dial，bind 成功 / 失败 / 不可达 / 仅 PAP 族放行）已在 M14.1（`ldapauth_test.go`）+ M14.2（`ldap_backend_test.go`、`ttls_inner_test.go` e2e）+ M14.3（`radius_auth_metrics_test.go`）分散交付，本项聚焦文档<br/>**已交付**：新增双语章节 `docs-site/src/{en,zh}/auth-ldap.md`「LDAP / AD Authentication Backend / 认证后端」，接入 `SUMMARY.md`（en/zh 均置于「运维指南」之后、「FAQ」之前）与中英 `documentation-map.md`。内容**全部 code-anchored**：①开篇即「仅 PAP 族」红线与原因（bind 需明文口令，挑战/响应类无法服务）；②「何时用/何时不用」诚实权衡（EAP-TTLS/PAP 在 TLS 隧道内传明文口令，安全性系于服务器证书 + TLS 1.2+，能出证书优先 EAP-TLS）；③部署模型（认证=LDAP bind、授权=本地 `RadiusUser` 行，LDAP 只换口令校验不开户，MAC 认证绕过）；④11 项 `ldap.*` 配置表（键/类型/默认/适用模式/说明，逐项对齐 `config_schemas.json`：`Enabled`/`ServerURL`/`BindMode`/`BindDNTemplate`/`BaseDN`/`UserFilter`(默认`(uid=%s)`)/`SearchBindDN`/`SearchBindPassword`/`StartTLS`/`TLSSkipVerify`/`Timeout`(默认5s,1–60)）；⑤template/search 两模式带 OpenLDAP + AD 实例；⑥传输安全（ldaps:// vs StartTLS、`TLSSkipVerify` 仅实验）；⑦认证流程表（裸 PAP / TTLS-PAP→bind；CHAP 系/EAP-MD5/PEAP→拒绝；MAC→绕过）+ 集中式失败即关闭设计说明；⑧安全模型（空凭证拨号前拒绝 RFC 4513 §5.1.2、`EscapeFilter`/`EscapeDN` 防注入、>1 命中拒绝、目录故障≠口令错误）；⑨可观测（`radus_reject_passwd_error` vs `radus_reject_ldap_error` vs `radus_accept` 区分语义）；⑩故障排查表。门禁：`mdbook build docs-site` 成功（双语页生成）；`lychee --offline`（CI 门禁，全书 0 错）+ `--include-fragments`（全书 0 错，校验内链锚点）通过；GitBook 经共享 SUMMARY 自动收录。
- [ ] M14.5（阻塞：待负载证据）连接健壮性：①连接复用/池化与重连——**当前每次认证新建连接**（M14.2 后端按尝试拨号），naive 池化**不安全**：连接 bind 身份随认证变化（template 模式以用户身份 bind；search 模式服务 bind 后再以用户身份 rebind），归还连接池会以错误身份服务下一请求；正确做法需「服务连接/用户连接」分离 + 归还前 rebind，属较大重构，且 LDAP 刚接入暂无负载证据→暂缓，待出现连接开销证据再做。②请求上下文传播——活 EAP 路径对 TTLS-PAP bind 传 `context.Background()`（`coordinator.go` 从不填 `EAPContext.Context`），无挂起风险（受 5s 超时约束）但请求级取消不传播；把真实请求 context 串入 `EAPContext.Context` 并在 `ttls_inner.go` 消费（已支持 nil 回退）。（**panic 恢复路径计数盲区已于 M14.8 交付**；**接受侧 `radus_accept` 计数器已于 M14.7 交付**，其引入的内存 sqlite UserRepo harness 可复用于后续接受路径单测。）
- [x] M14.6 `test/integration/` LDAP 端到端验收用例（CI 自动执行）：以 OpenLDAP service 容器（仿照 integration job 的 postgres service）+ 种子目录账号，让已启动的 RadiusService 经真实 bind 完成裸 PAP 与 `EAP-TTLS/PAP` 认证；覆盖 bind 成功放行、口令错误拒绝、目录不可达→`radus_reject_ldap_error`、非 PAP 族拒绝。**已交付**：新增 `test/integration/ldap_test.go`（`//go:build integration`，package `integration`，CI `Integration (PostgreSQL/OpenLDAP)` job 自动执行），并把共享 integration job / `docker-compose.test.yml` / `make test-integration-pg` 扩展为 PostgreSQL + OpenLDAP 双服务。测试使用 `bitnami/openldap:2.6` 中的真实目录账号（CI：`alice/alice-password`，`dc=example,dc=org`），通过 `ldap.BindMode=search`（服务账号 bind→`(cn=%s)` 搜索→用户 bind）接入生产 `ldapauth.Verifier`。为证明不是本地口令误通过，本地 `RadiusUser.Password` 固定写成 `local-password-must-not-be-used`，只保留授权属性（profile、状态、到期等），成功必须来自 OpenLDAP bind。验收覆盖：①裸 PAP 正确目录口令→Access-Accept；②裸 PAP 错误口令→Access-Reject 且 `radus_reject_passwd_error` +1；③LDAP URL 临时切到空闲端口模拟目录不可达→Access-Reject、`radus_reject_ldap_error` +1、Reply-Message 含 `ldap backend unavailable`；④裸 CHAP（无 User-Password）在 LDAP 启用时明确拒绝，`radus_reject_ldap_error` +1，Reply-Message 含 `no PAP password` / `LDAP bind` 诊断；⑤`EAP-TTLS/PAP` 经真实 UDP RADIUS + TLS 1.2 外层隧道 + 内层 PAP AVP，以同一目录口令完成 bind→Access-Accept + EAP-Success + MS-MPPE 双钥。配套修复一处代码注释漂移：`internal/ldapauth` 包注释不再声称“尚未接入活路径”，改为说明 live RADIUS path 通过 `internal/radiusd` 适配 Verifier。
- [x] M14.7 接受侧可观测计数器（`radus_accept` 自增）——**已交付**：M14.3 修好了拒绝侧按原因计数，但**接受侧对称缺口仍在**——`auth_stages.go::sendAcceptResponse` 此前仅把 `MetricsRadiusAccept` 当作 zap 日志字段输出、从不调用 `IncRadiusMetric`，故 `radus_accept` 恒为 0，认证成功率/SLO 无法计算（M14.4 文档当时只能诚实记为「尚未自增」）。全仓审计确认接受出口唯一：EAP 成功（`auth_stages.go:165`）与裸 PAP/CHAP 成功（`:268`）两路**互斥**（EAP 成功后 `ctx.Stop()` 使 `stagePluginAuth` 因 `IsStopped` 提前返回），均汇聚到 `sendAcceptResponse`；`SendAccept`/`SendEAPSuccess` 仅在此调用→**单点计数、不重复**。修复=在 nil 守卫之后、成功日志之前补一行 `app.IncRadiusMetric(app.MetricsRadiusAccept)`（每次成功认证恰好自增一次，且上下文缺失被守卫拦下时不计数）。测试（变异验证）：新增 `auth_accept_metrics_test.go`（内存 sqlite UserRepo + 空注册表使 enhancer 安全空转）——裸/EAP 两路各自增一次、上下文缺失不自增也不发包；删除自增行→测试失败（已验证）。配套修正双语 `auth-ldap.md` 可观测小节（由「尚未自增」改为「每次成功自增一次、可与拒绝计数器配对算成功率」），与 M14.4 保持一致。门禁：`gofmt`、`CGO_ENABLED=0 go build/test ./...`、golangci-lint v2.12.2 全绿；`mdbook build` + `lychee --offline --include-fragments` 0 错。**至此 `radus_accept` 与 `radus_reject_*` 构成完整的成功/失败计数对。**
- [x] M14.8 panic 恢复路径计数盲区（`radus_auth_drop` 自增）——**已交付**：续完可观测三件套的最后一块。`ServeRADIUS` 的 `defer recover()` 分支（`radius_auth.go:39-58`）此前把 `MetricsRadiusAuthDrop` 仅作为 zap 日志字段输出、随后直接调 `SendReject` 应答，**从不调用 `IncRadiusMetric`**——故被 recover 的 panic 请求不自增 `radus_auth_drop`，崩溃风暴只见于日志、不见于计数器（与 M14.3 裸拒绝、M14.7 接受侧同类盲区）。修复=在该恢复分支的日志之后、`SendReject` 之前补一行 `app.IncRadiusMetric(app.MetricsRadiusAuthDrop)`。**不重复计数**：panic 恢复路径绕过常规拒绝出口（`logAndReject`），是唯一对 panic 计数的点；`Execute`（`auth_pipeline.go`）内部不 recover，stage panic 直冒泡到 `ServeRADIUS`。测试（变异验证）：新增 `radius_panic_metrics_test.go`——注入一个仅含 panic stage 的流水线（`ensurePipeline` 保留已注入流水线），断言 `ServeRADIUS` 恢复后 `radus_auth_drop` 恰好 +1 且仍向客户端回 Access-Reject；删除自增行→测试失败（已验证）。门禁：`gofmt`、`CGO_ENABLED=0 go build/test ./...`、golangci-lint v2.12.2 全绿（未触前端/文档）。**至此 `radus_accept`（M14.7）/ `radus_reject_*`（M14.3）/ `radus_auth_drop`（M14.8）构成接受/拒绝/异常丢弃的完整可观测三件套。**
...
- 选任务口径：优先取**优先级最高里程碑**中自上而下第一个未勾选、且未标注 `阻塞` / `待证据` 的 `- [ ] M*.*`（优先级见里程碑总览的优先级列与上面的调度优先级排期）；同优先级里程碑按序号顺序。agent 在**本机**用你自己的 agent 运行，不在 CI 执行；运行参考与护栏见 [`.agents/README.md`](../.agents/README.md)。
