...
- [x] M13.5 GitHub Pages 部署工作流 —— 新增 `.github/workflows/pages.yml`：`peaceiris/actions-mdbook`（钉 0.5.3，与 CI/本地一致）构建 `docs-site` → `actions/upload-pages-artifact@v3` → `actions/deploy-pages@v4`（OIDC，`permissions: pages:write/id-token:write`，`concurrency: pages`）。触发：push `main` 命中 `docs-site/**` 或本工作流 + 手动 `workflow_dispatch`。仓库 Pages 源须为「GitHub Actions」（REST `build_type: workflow`，现状已是）。**修复「Pages 无法访问」根因**：此前 Pages 已配 `build_type=workflow` 却无任何部署工作流，站点自 2025-11 起未再发布（`www.toughradius.net`/`talkincode.github.io` 返回 404）。手册经仓库既有 Pages 域名配置对外服务；工作流不管理自定义域名，保留既有配置不与 GitBook 冲突（见 `gitbook-coexistence`）。actionlint 通过。
