...
| M10 | EAP-TLS 1.3 / RFC 9190 upgrade | TR-F004 | P2 | In progress (M10.1 #562, M10.2 #564, M10.3 #607 delivered; next M10.5 identity protection / anonymous NAI) |
...
- [x] M10.3 Tolerate the peer's TLS `close_notify` after the EAP-TLS success point. Delivered (#607): after the success point (`stateKeyPendingSuccess`) a real supplicant usually closes its TLS stack — emitting a `close_notify` warning alert — instead of returning the bare ACK of RFC 9190 Figure 1; the old code rejected that non-ACK fragment as `ErrTLSUnexpectedFragment`, turning a successful authentication into an Access-Reject. The fix is **pure-EAP-TLS only** (guarded by `onApplicationData == nil`; PEAP/TTLS keep strict handling): a non-ACK fragment is routed to a new `finishAfterPeerClosure`, which reads via `engine.ReadApplication` — `io.EOF` (clean `close_notify`) → grant and still export the MS-MPPE keys; a non-EOF read error (a TLS *error* alert) → reject `ErrTLSHandshakeFailed`; stray application data → reject `ErrTLSUnexpectedFragment` (fail-closed). RFC-anchored: RFC 9190 §2.1.4 (`close_notify` is a warning-level closure, not an error), §2.5 (only TLS *error* alerts are failure result indications), RFC 8446 §6.1. Four unit tests (1.2 close→grant, 1.3 read-0x00-then-close→grant, stray app-data→reject, corrupt record→reject) plus a `test/integration/` subtest (`tls 1.2 client close_notify still authenticates`, CI-backed); green under `-race`. Identity protection (anonymous NAI) is split to **M10.5** because it needs an auth-pipeline reorder.
- [x] M10.4 Unit + `test/integration/` TLS 1.3 end-to-end acceptance (CI-executed). Satisfied incrementally by M10.1–M10.3: `test/integration/eap_tls_test.go` now carries 6 CI subtests covering TLS 1.3/1.2 negotiation + fallback, MSK → MS-MPPE derivation, and `close_notify` authentication, each landed with its feature subtask and run automatically in CI; no distinct deliverable remains, so this is folded closed.
- [ ] M10.5 Identity protection / anonymous NAI (RFC 9190 §2.1.7 / §2.1.8): let the peer present an anonymous NAI (e.g. `anonymous@realm`) in the outer EAP-Identity and have the server resolve the real account from the **client certificate** inside the TLS tunnel rather than the outer User-Name. Requires an auth-pipeline reorder: `stageLoadUser` currently resolves the account by the outer User-Name before EAP runs, so account binding/load must move to after the certificate identity is available from the TLS handshake. Keep backward compatibility with non-anonymous clients; unit tests + a `test/integration/` anonymous-NAI end-to-end case (CI-backed).
