...
- [ ] M8.3a `tlsengine` 隧道建立后应用数据收发（`WriteApplication`/`ReadApplication`）与 TLS 密钥材料导出（RFC 5705 `ExportKeyingMaterial`，供 MPPE/MSK 派生）；握手完成后保持引擎存活
- [ ] M8.3b 隧道内 EAP-MSCHAPv2 交换（复用 mschapv2 校验）与 MPPE 密钥导出（基于 M8.3a 能力，隧道内跑内层 EAP，认证成功后从 TLS 导出 MSK 下发 MS-MPPE-Send/Recv-Key）
