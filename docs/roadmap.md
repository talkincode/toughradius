...
- [x] M6.5 补全计费成功计数器 `radus_accounting`：常量已在 `internal/app/radius_metrics.go` 定义并纳入 `metricsNames`，但计费成功落地路径从未调用 `IncRadiusMetric(MetricsRadiusAccounting)`——成功计费请求量当前不可见（认证侧 M14.7 `radus_accept` 的计费侧对应物）。需在计费请求成功处理处自增并补单测。发现来源：M6.1 实现。<br/>**已交付**：`AcctService.ServeRADIUS` 在 Accounting-Request 通过 NAS/用户名/secret 校验且 Accounting-Response 成功写出后自增 `radus_accounting`，并保留后台插件处理失败不重复污染成功计数的边界；`SendResponse` 返回写出结果，使响应写失败仍只计入 `radus_acct_drop`。补 `internal/radiusd/radius_acct_metrics_test.go` 覆盖成功接收只自增一次、响应写失败不计入成功指标。
...
- [x] M6.7 计费 panic 路径补 `radus_acct_drop` 自增：`ServeRADIUS` 的 recover 块当前仅输出 `metrics: radus_acct_drop` 日志字段却未调用 `IncRadiusMetric`，panic 诱发的丢弃仍不可见。需在 recover 路径补自增，并以可诱发 panic 的测试守卫。发现来源：M6.1 独立评审 gate。<br/>**已交付**：recover 分支在记录 panic 日志前自增 `radus_acct_drop`，不改变正常错误流、工作池饱和或响应写失败的既有分类；补 `TestServeRADIUSCountsDropOnPanicRecover` 诱发可控 panic 并断言 drop 计数自增。
...
- [ ] M14.5 连接健壮性（证据驱动，待负载证据）：①连接复用/池化与重连——**当前每次认证新建连接**（M14.2 后端按尝试拨号），naive 池化**不安全**：连接 bind 身份随认证变化（template 模式以用户身份 bind；search 模式服务 bind 后再以用户身份 rebind），归还连接池会以错误身份服务下一请求；正确做法需「服务连接/用户连接」分离 + 归还前 rebind，属较大重构，且 LDAP 刚接入暂无负载证据→暂缓，待出现连接开销证据再做。②请求上下文传播——活 EAP 路径对 TTLS-PAP bind 传 `context.Background()`（`coordinator.go` 从不填 `EAPContext.Context`），无挂起风险（受 5s 超时约束）但请求级取消不传播；把真实请求 context 串入 `EAPContext.Context` 并在 `ttls_inner.go` 消费（已支持 nil 回退）。③panic 恢复路径计数盲区——`ServeRADIUS` 的 panic 恢复分支（`radius_auth.go:50-56`）直接调 `SendReject` 而非 `logAndReject`，故 panic 恢复不自增 `radus_auth_drop`（与 M14.3 同类盲区）；干净单测需诱发 panic 故单列。（**接受侧 `radus_accept` 计数器已于 M14.7 交付**；其引入的内存 sqlite UserRepo harness 可复用于后续接受路径单测。）
