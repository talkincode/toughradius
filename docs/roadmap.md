...
- [ ] M14.5 连接健壮性（证据驱动，待负载证据）：①连接复用/池化与重连——**当前每次认证新建连接**（M14.2 后端按尝试拨号），naive 池化**不安全**：连接 bind 身份随认证变化（template 模式以用户身份 bind；search 模式服务 bind 后再以用户身份 rebind），归还连接池会以错误身份服务下一请求；正确做法需「服务连接/用户连接」分离 + 归还前 rebind，属较大重构，且 LDAP 刚接入暂无负载证据→暂缓，待出现连接开销证据再做。②请求上下文传播——活 EAP 路径对 TTLS-PAP bind 传 `context.Background()`（`coordinator.go` 从不填 `EAPContext.Context`），无挂起风险（受 5s 超时约束）但请求级取消不传播；把真实请求 context 串入 `EAPContext.Context` 并在 `ttls_inner.go` 消费（已支持 nil 回退）。③接受路径指标配套——`sendAcceptResponse` 同样从不自增 `MetricsRadiusAccept`（仅写 zap 日志字段），故认证成功率无法计算；补 `IncRadiusMetric(MetricsRadiusAccept)`；同理 `ServeRADIUS` 的 panic 恢复分支（`radius_auth.go:50-56`）直接调 `SendReject` 而非 `logAndReject`，故 panic 恢复也不自增 `radus_auth_drop`（与 M14.3 修复同类盲区，一并补）；接受路径会触达 DB（`UpdateBind`/`UpdateUserLastOnline`），干净单测需先引入 mock UserRepo harness。
