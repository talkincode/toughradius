...
func listNodes(c echo.Context) error {
	...
	base := GetDB(c).Model(&domain.NetNode{})
...
func getNode(c echo.Context) error {
	...
	if err := GetDB(c).Where("id = ?", id).First(&node).Error; errors.Is(err, gorm.ErrRecordNotFound) {
...
func createNode(c echo.Context) error {
	...
	var exists int64
	GetDB(c).Model(&domain.NetNode{}).Where("name = ?", payload.Name).Count(&exists)
	if exists > 0 {
	...
	if err := GetDB(c).Create(&node).Error; err != nil {
...
func updateNode(c echo.Context) error {
	...
	if err := GetDB(c).Save(&node).Error; err != nil {
...
func deleteNode(c echo.Context) error {
	...
	var nasCount int64
	GetDB(c).Model(&domain.NetNas{}).Where("node_id = ?", id).Count(&nasCount)
	if nasCount > 0 {
