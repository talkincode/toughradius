...
func (s *FreeradiusServer) Start() error {
	zap.S().Infof("Starting Freeradius API server %s:%d", app.GConfig().Freeradius.Host, app.GConfig().Freeradius.Port)
	err := s.root.Start(fmt.Sprintf("%s:%d", app.GConfig().Freeradius.Host, app.GConfig().Freeradius.Port))
	if err != nil {
		zap.S().Errorf("Starting Freeradius API server error %s", err.Error())
	}
	return err
}
