...
func TestParseVlanIds(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Standard format 1 - with port and VLAN",
			input:   "3/0/1:2814.727",
			vlanid1: 2814,
			vlanid2: 727,
		},
		{
			name:    "Standard format 1 - VLAN only",
			input:   "3/0/1:2814",
			vlanid1: 2814,
			vlanid2: 0,
		},
		{
			name:    "Standard format 2 - slot/subslot/port",
			input:   "slot=2;subslot=2;port=22;vlanid=503;",
			vlanid1: 503,
			vlanid2: 0,
		},
		{
			name:    "Standard format 2 - dual VLAN",
...
func TestParseVlanIdsOutput(t *testing.T) {
	// Preserve the original output test
	s := "3/0/1:2814.727"
...
func TestParseVlanIdsEdgeCases(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Negative VLAN (invalid)",
			input:   "3/0/1:-1",
			wantErr: false, // Parse failure should return 0
		},
		{
			name:    "Oversized VLAN ID",
			input:   "3/0/1:99999",
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			vlanid1, vlanid2 := ParseVlanIds(tt.input)
			// Validation-only mode should not panic
...
func TestParseVlanIdsRegexMatch(t *testing.T) {
	// Test regex pattern matching for different formats
	inputs := []string{
		"1/0/1:100",
		"2/1/10:200.300",
	...
	for _, input := range inputs {
		t.Run("Input_"+input, func(t *testing.T) {
			...
			// Validate the results for reasonableness
			if vlanid1 < 0 || vlanid1 > 4094 {
				if vlanid1 != 0 { // 0 indicates no match, which is acceptable
