...
const (
	EAPHeaderLength        = 4
	EAPMD5ChallengeLength  = 16
	EAPOTPChallengeMessage = "Please enter a one-time password"
)
...
func NewEAPSuccess(identifier uint8) *EAPHeader {
	return &EAPHeader{
		...
		Length:     EAPHeaderLength, // EAP header is always 4 bytes for Success/Failure
	}
}
...
func generateRandomBytes(n int) ([]byte, error) {
	...
	// 注意这里返回的 n 是读取的字节数
	if err != nil {
		zap.S().Error("Failed to generate random bytes", zap.Error(err))
		return nil, err
	}
	return b, nil
}
...
func (s *AuthService) sendEapChallengeRequest(w radius.ResponseWriter, r *radius.Request, secret string, eapType uint8, eapChallenge []byte, method string) error {
	// 创建一个新的RADIUS响应
	var resp = r.Response(radius.CodeAccessChallenge)

	state := common.UUID()
	s.AddEapState(state, rfc2865.UserName_GetString(r.Packet), eapChallenge, method)
	...
	eapMessage = append(eapMessage, eapType)               // Type for EAP
...
func (s *AuthService) sendEapMD5ChallengeRequest(w radius.ResponseWriter, r *radius.Request, secret string) error {
	eapChallenge, err := generateRandomBytes(EAPMD5ChallengeLength)
	if err != nil {
		return err
	}
	return s.sendEapChallengeRequest(w, r, secret, EAPTypeMD5Challenge, eapChallenge, EapMd5Method)
}

func (s *AuthService) sendEapOTPChallengeRequest(w radius.ResponseWriter, r *radius.Request, secret string) error {
	eapChallenge := []byte(EAPOTPChallengeMessage)
	return s.sendEapChallengeRequest(w, r, secret, EAPTypeOTP, eapChallenge, EapOTPMethod)
}
