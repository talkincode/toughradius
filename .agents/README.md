...
- **质量门禁**：`go build ./...`、`go test ./...`、`golangci-lint run`（v2.12.2）通过；前端改动跑 `cd web && npm run build`；workflow / actionlint 配置改动必须确认 `Workflow Lint` 通过（`actionlint -shellcheck=`，范围限于 GitHub Actions YAML、表达式与 action 输入）。
