...
// configureTTLS stores the server certificate/key as a managed certificate and
// points the dynamic EAP settings at it by name, switching the server's EAP
// method to eap-ttls. EAP-TTLS is a server-only TLS method, so no client-CA
// bundle is required (unlike EAP-TLS). The settings provider resolves the managed
